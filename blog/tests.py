...
class TextView(TestCase):
 ...
 #       self.assertIn(self.post_001.content, post_area.text)
    ...
    def test_update(self):
        ...
        self.assertNotEqual(response.status_code, 200)  #로그인하지 않았을 경우 
        ...
        response = self.client.get(update_post_url)
        self.assertEqual(response.status_code, 403) #권한이 없는 사람이 접근했을 때 오류코드 403이 잘 작동 되나 확인한다.

        #작성자(obama)가 접근하는 경우
        self.client.login(
            username = self.post_003.author,
        ...
        tag_str_input = main_area.find('input', id='id_tags_str')
        self.assertTrue(tag_str_input)
        self.assertIn('파이썬 공부; python', tag_str_input.attrs['value']) #기존 태그는 합격
...
def test_comment_form(self):
    ...
    comment_area = soup.find('div', id='main-area')
    ...
    comment_form = soup.find('div', id="comment-form")
...
def test_delete_comment(self):
    comment_by_trump = Comment.object.create(
        post=self.post_001,
        author=self.user_trump,
        content='트럼프의 댓글입니다.',
    )
    ...
    comment_area = soup.find('div', id="comment-area")
    self.assertIn("트럼프의 댓글입니다.", comment_area.text)
